...
final class WhenCreatingWorkoutObject: XCTestCase {
    ...
    func testShouldCreateWorkoutObjectUsingNumRoundsSuperSetInit() throws {
        ...
        XCTAssertEqual(workout.supersets[0].sets[0].count, 2)
        XCTAssertEqual(workout.supersets[0].sets[0][0].exercise.name, "Squat")
        XCTAssertEqual(workout.supersets[1].sets[0][0].exercise.name, DatabaseExercise.sampleExercises[0].name)
    }
    ...
    func testShouldCreateWorkoutObjectUsingExplicitSuperSetInit() throws {
        ...
        let superSet1 = SuperSet(sets: [[set1, set2], [set3,set4]], rest: 50)
        ...
        let superSet2 = SuperSet(sets: [[set5, set6], [set7,set8]], rest: 120)
        let workout = Workout(supersets: [superSet1, superSet2])
        XCTAssertEqual(workout.supersets.count, 2)
        XCTAssertEqual(workout.supersets[0].sets.count, 2)
        XCTAssertEqual(workout.supersets[0].sets[0].count, 2)
        XCTAssertEqual(workout.supersets[1].sets[1][0].weight, 120)
        XCTAssertEqual(workout.supersets[1].sets[1][0].exercise.name, "Deadlift")
        XCTAssertEqual(workout.supersets[1].sets[1][1].exercise.name, DatabaseExercise.sampleExercises[1].name)
