...
struct EditWorkoutView: View {
    ...
    init() {
        let set1 =  SingleSet(exercise: AnyExercise(.databaseExercise( exercises.first(where: { $0.id == "Barbell_Squat" }) ?? exercises[0])), weight: 100, reps: 5)
        let set2 = SingleSet(exercise: AnyExercise(.databaseExercise( exercises.first(where: { $0.id == "Bench_Press_-_Powerlifting" }) ?? exercises[0])), weight: 50, reps: 6)
        let set3 = SingleSet(exercise: AnyExercise(.databaseExercise( exercises.first(where: { $0.id == "90_90_Hamstring" }) ?? exercises[0])), weight: 40, reps: 9)
